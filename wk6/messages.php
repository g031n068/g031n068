...
<html lang="ja">
<head>
  <title><?php echo htmlspecialchars($_GET['n'])?></title>
...
<body>
  ...
  <section>
    ...
    if ( $link !== false ) {
        ...
        if ( isset( $_POST['send'] ) === true ) {
            ...
            $thnum = htmlspecialchars($_POST['thnum']);

            if ( $name !== '' && $body !== '' && $pass !== '' ) {

                //INSERT文の設定
                $query = " INSERT INTO messages ( "
                       . " name,password,body,thread_num "
                       . " ) VALUES ( "
                       . "'" . mysqli_real_escape_string( $link, $name ) ."',"
                       . "'" . mysqli_real_escape_string( $link, $pass ) ."',"
                       . "'" . mysqli_real_escape_string( $link, $body ) ."',"
                       . "'" . mysqli_real_escape_string( $link, $thnum ) ."'"
                       ." ) ";
        ...
        //SELECT文の設定
        //$query  = "SELECT * FROM threads INNER JOIN messages ON threads.thread_id = messages.threads_num WHERE thread_id = $_GET['id'] ";
        //$name = $_GET['n']
        $id = htmlspecialchars($_GET['id']);
        $query  = "SELECT * FROM messages WHERE thread_num = $id ORDER BY message_id DESC";
        $res    = mysqli_query( $link,$query );
  ...
  <article>
    <h1><?php echo htmlspecialchars($_GET['n'])?></h1>
    ...
    <?php foreach( $res as $row ): ?>
              <tr><td width="100" align="center"><?php echo htmlspecialchars($row['name'])?></td>
                  ...
                  <td>
                      ...
                      <input type="hidden" name="thnum" value="<?php echo htmlspecialchars($_GET['id'])?>">
                      <input type="hidden" name="thname" value="<?php echo htmlspecialchars($_GET['n'])?>">
